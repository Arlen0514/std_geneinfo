$(function () {
    ...
    var $tabs = $('.index_newsBg ul.tabs button[role="tab"]');
    var $panels = $('.index_newsBg .p_tab_text_area [role="tabpanel"]');

    var $mobileTabBtn = $('.index_newsBg .tabs_btn_now_arrow');
    var $imgScroll = $('.index_newsBg .img-scroll');
    ...
    function switchTab($tab, moveFocusToContent) {
        ...
        //     ? $('.index_honorBg ul.tabs button[role="tab"]').first()
        //     : $tabs.eq(currentTabIndex + 1);


        var $nextFocus = isLastTab
            ? $('.index_honorBg .honor_content').first()
            : $tabs.eq(currentTabIndex + 1);

        // 固定入口：swiper 第一個真實 slide 的 a（排除 loop clone）
        var $entryLink = $targetPanel
            .find('.swiper-slide:not(.swiper-slide-duplicate) a')
            .first();

        // fallback：沒有輪播時用第一個可見的 a 或 button
        if (!$entryLink.length) {
            $entryLink = $targetPanel.find('a, button').filter(':visible').first();
        }

        // 固定出口：.index_news_more a
        var $exitLink = $targetPanel.find('.index_news_more a');
        ...
        // panel 出口 Tab → 下一個 tab button（最後一個 tab 則跳到 .index_honorBg 第一個 tab）
