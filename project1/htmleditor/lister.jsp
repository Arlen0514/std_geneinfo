...
<script language="javascript" type="text/javascript" src="<%=request.getContextPath()%>/htmleditor/tinymce/tinymce.min.js"></script>
...
function selectFile(URL) {
    window.opener.postMessage(URL, window.opener.location.origin);
    window.close();
}
</script>
...
<%
	...
	// 如果有除了 MIS 外的後臺，要在加上其他資料夾 (例如 ： SCM )
	String xid  =  StringTool.validString(request.getParameter("id"),"");
	if((String)session.getAttribute("path_id")!= null){
		xid = (String)session.getAttribute("path_id");
	}
...
%>
<script type="text/javascript">
	  selectFile('<%=contextPath+subpath+"/"+filename %>');
</script>
<%
	}
...
for (int i = 0; i < fAllFiles.length; i++) {
	if (fAllFiles[i].isFile()) {
		...
		String[] imagType={".jpg",".png",".bmp",".gif",".webp"};
...
%> 
                                    <!--清單-->
                                    <div class="file_list">
                                    	<div class="file_listIn" onClick="selectFile('<%=contextPath + subpath + "/" + fAllFiles[i].getName()%>');">
