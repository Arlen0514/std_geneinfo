...
<%
    ...
    int choose_num = 10;             // 可選取筆數
/*--------------------------------------------------------------------------------------------*/
	// 已選取列表
	Vector pds = app_sm.selectAll(tblpd, "pd_code=? and pd_lang=? and pd_hot like ? ", new Object[] { data_code, lang , "Y%" }, "pd_hot ASC ");
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0">
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						for (int i = 0; i < choose_num; i++) {
							TableRecord pd = app_sm.select(tblpd, "pd_code=? AND pd_lang=? AND pd_hot=?", new Object[]{ data_code , lang ,"Y"+(i+1)});
							// 已選取列表
							if(!pd.getString("pd_id").isEmpty()){
						%>
						<form name="list<%=i + 1%>" id="list<%=i + 1%>" method="post">
						<tr class="web_table-2-1">
							<td align="center"><%=((pageno - 1) * page_items) + i + 1%></td>
							<td align="center"><%=app_sm.select(tbldm, pd.getString("pd_category")).getString("dm_title")%></td>
							<td align="center"><img src="<%=app_fetchpath+"/"+data_code+"/"+lang+"/"+pd.getString("pd_image")%>" width="200px"></td>						
							...
							<td align="center">
								...
								<input type="hidden" name="pd_replaceid" id="pd_replaceid" value="<%=pd.getString("pd_id")%>" />
								<input name="showno" type="hidden" value="<%=i+1%>">
								<input name="modify<%=i+1%>" type="button" value="重新選取" onClick="goaction(this.form, '<%=code%>_b.jsp');">
                       			<input name="modify<%=i+1%>" type="button" value="清除選取" onClick="goaction(this.form, '<%=code%>_update.jsp?action=clear');">							
							</td>
						</tr>
						</form>
						<% 	}else{ // 未選取列表 %>
						...
						<tr class="web_table-2-1">
							...
							<td align="center">
								<%=HtmlCoder.hiddenInputs(names, values)%>
								<input name="showno" type="hidden" value="<%=i+1%>">
								<input type="button" value="選取" onclick="goaction(this.form, '<%=code%>_b.jsp');" />								
							</td>
						</tr>
						</form>
						<% 
