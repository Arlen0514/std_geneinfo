		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()) {
				...
				if(fieldName.equals("sp_title")) {
					/*
					Vector checks = app_sm.selectAll(tblsp,"sp_title =? and sp_code=? and sp_lang=?",new Object[]{fieldvalue, code, lang});
					if(checks.size()>0){
	...
	// 修改
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()) {
				...
				if(fieldName.equals("sp_title")) {
					Vector checks = app_sm.selectAll(tblsp,"sp_title =? and sp_id <>? and sp_code=? and sp_lang=?",new Object[]{fieldvalue, sp_id, code, lang});
					/*
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
