	...
	if("D".equals(action)) {
		...
		if (app_sm.success()) {
			if(app_sm.selectAll(tblfr, "fr_image=?", new Object[] { fr.getString("fr_image") }).size() == 0) {
				FileTool.deleteFile(app_uploadpath+"/"+upload_code+"/"+lang+"/"+fr.getString("fr_image"));
			}
			// 回列表頁
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("fr_title")){
			    	...
			    	String imgradio = fieldvalue.trim();
					if (imgradio.equals("delete")) {
						//確認沒有其它資料使用此圖檔
						if(app_sm.selectAll(tblfr,"fr_image=? and fr_id<>?",new Object[]{fr.getString("fr_image"),fr.getString("fr_id")}).size()==0 ){
							FileTool.deleteFile(app_uploadpath + "/" + upload_code + "/" + lang + "/" + fr.getString("fr_image"));
						}
						fr.setValue("fr_image","");
						imgradio = "";
					}
					...
					*/
					...
					if(!"".equals(fr.getString(fi.getFieldName()))){
						...
						if(app_sm.selectAll(tblfr, fi.getFieldName()+"=?", new Object[] { fr.getString(fi.getFieldName()) }).size() == 1) {
							FileTool.deleteFile(app_uploadpath+"/"+upload_code+"/"+lang+"/"+fr.getString(fi.getFieldName()));
						}
					}
