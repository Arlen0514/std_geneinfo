...
/*------------------------------------------------------------------------------------*/	
	...
	// 所屬類別標題
	String title1=app_sm.select(tbldm,fd.getString("fd_upcategory")).getString("dm_title");
	String title2=app_sm.select(tbldm,fd.getString("fd_category")).getString("dm_title");
...
<%-- 20250331 modify Miles top menu.jsp  改成 RWD --%>
			...
			<tr>
				...
				<form name="frm" id="frm" method="post" enctype="multipart/form-data" action="<%=code%>_update.jsp?action=M&fd_id=<%=fd_id %>&_qtitle=<%=qtitle %>&npage=<%=pageno %>&_qcategory=<%=qcategory %>&_qupcategory=<%=qupcategory %>" onsubmit="javascript:return checkform(this);">
							...
							<tr class="web_table-2-1">
								...
								<td colspan="3" width="85%" align="left">
									...
									<span id="dm_title" style="color:#FA0300;"><%=title1+"--"+title2%></span>&nbsp;&nbsp;
                   					<input type="hidden" name="fd_upcategory" id="fd_upcategory"  value="<%=fd.getString("fd_upcategory") %>"/>
									<input type="hidden" name="fd_category" id="fd_category"  value="<%=fd.getString("fd_category") %>"/>
                  					<input type="button" value="選擇類別" onclick="window.open('../mis_tools/category_selector.jsp?Back_id=fd_category&Back_category=fd_upcategory&max_layer=<%=category_num %>&dm_code=<%=code%>_category','_blank','height=260,width=400,top=50,left=300,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');" />
                  					<%} %>
                  					<%--
									//兩層下拉
									<select name="fd_category" id="fd_category">
										...
										<optgroup label="<%=dm.getString("dm_title")%>">
											...
											%>
											<option value="<%=sub_dm.getString("dm_id")%>" <%=fd.getString("fd_category").equals(sub_dm.getString("dm_id")) %>><%=sub_dm.getString("dm_title")%></option>
											<%} %>
										</optgroup>
										<%} %>
									</select>
