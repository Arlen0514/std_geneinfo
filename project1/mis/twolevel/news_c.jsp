...
/*-------------------------------------------------------------------------*/
	...
	// 跳頁參數
	String[] names = new String[] { "npage", "_qtitle", "_qcategory", "_qemitdate", "_qrestdate" , "_qupcategory"  };
	String[] values = new String[] { String.valueOf(pageno), qtitle, qcategory, qemitdate, qrestdate , qupcategory  };
	// 回列表頁
	...
	// 所屬類別標題
	String title1=app_sm.select(tbldm,np.getString("np_upcategory")).getString("dm_title");
	String title2=app_sm.select(tbldm,np.getString("np_category")).getString("dm_title");
%>
...
<%-- 20250331 modify Miles top menu.jsp  改成 RWD --%>
			...
			<tr>
				...
				<form name="frm" id="frm" method="post" enctype="multipart/form-data" action="<%=code%>_update.jsp?action=M&np_id=<%=np_id %>&_qtitle=<%=qtitle %>&npage=<%=pageno %>&_qcategory=<%=qcategory %>&_qemitdate=<%=qemitdate %>&_qrestdate=<%=qrestdate %>&_qupcategory=<%=qupcategory %>" onsubmit="javascript:return checkform(this);">
						...
						<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
							...
							<tr class="web_table-2-1">
								...
								<td colspan="3" width="85%" align="left">
									...
									<span id="dm_title" style="color:#FA0300;"><%=title1+"--"+title2%></span>&nbsp;&nbsp;
                   					<input type="hidden" name="np_upcategory" id="np_upcategory"  value="<%=np.getString("np_upcategory") %>"/>
									<input type="hidden" name="np_category" id="np_category"  value="<%=np.getString("np_category") %>"/>
                  					<input type="button" value="選擇類別" onclick="window.open('../mis_tools/category_selector.jsp?Back_id=np_category&Back_category=np_upcategory&max_layer=<%=category_num %>&dm_code=<%=code%>_category','_blank','height=260,width=400,top=50,left=300,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');" />
                  					<% } %>
                  					<%--
									//兩層下拉
									<select name="np_category" id="np_category">
										...
										<optgroup label="<%=dm.getString("dm_title")%>">
											...
											%>
											<option value="<%=sub_dm.getString("dm_id")%>" <%=np.getString("np_category").equals(sub_dm.getString("dm_id")) %>><%=sub_dm.getString("dm_title")%></option>
											<%} %>
										</optgroup>
										<%} %>
									</select>
		                    ...
		                    <tr class="web_table-2-1">
								<td colspan="3" align="left" class="tablebg">
									...
									<br />
								    <input name="imgradio" type="radio" value="delete" onclick="frm.np_image.value='';">刪除檔案
								</td>
		                    </tr>
