...
<%
	...
	int category_num = 2;		    	// 設定商品類別層數 2層以上使用選擇器 一層使用下拉選單即可
...
/*--------------------------------------------------------------------------------------*/	
	...
	// 所屬類別標題
	String title1=app_sm.select(tbldm,pd.getString("pd_upcategory")).getString("dm_title");
	String title2=app_sm.select(tbldm,pd.getString("pd_category")).getString("dm_title");
...
function checkform(F){	
	...
	if (F.pd_category.value == "") {
    	...
    	<%-- 舊圖資訊 故不須上傳 記錄檔名即可--%>
    	if (F.pd_image.value == "") {
    		F.pd_image.type = "text";
    		F.pd_image.value = "<%=pd.getString("pd_image")%>";
    	}
    	<%-- 新圖資訊 須將舊資料清除 --%>
    	<%for(int imgCount = 1; imgCount <= otherimage_count; imgCount++) {%>
    	if (typeof F.imgradio<%=imgCount %> != "undefined" && F.imgradio<%=imgCount %>.value != "ucpic") {
    		F.pd_oldImage<%=imgCount %>.value = "";
    	}
    	<%}%>
...
<%-- 20250331 modify Miles top menu.jsp  改成 RWD --%>
		<form name="form0" id="frm" method="post" enctype="multipart/form-data" action="<%=code%>_update.jsp?action=AC" onsubmit="javascript:return checkform(this);">
						...
						<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
								...
								<td colspan="3" align="left">
								    ...
								    <select name="pd_category" id="pd_category">
										...
										<% for(TableRecord dm : dms){ %>
											<option value="<%=dm.getString("dm_id")%>" <%=dm.getString("dm_id").equals(pd.getString("pd_category"))?"selected":""%>><%=dm.getString("dm_title")%></option>
										<% } %>
									</select>
									<% }else{  //2層以上 類別選擇器 %>
									<span id="dm_title" style="color:#FA0300;"><%=title1+"--"+title2%></span>&nbsp;&nbsp;
                   					<input type="hidden" name="pd_upcategory" id="pd_upcategory"  value="<%=pd.getString("pd_upcategory") %>"/>
									<input type="hidden" name="pd_category" id="pd_category"  value="<%=pd.getString("pd_category") %>"/>
                  					<input type="button" value="選擇類別" onclick="window.open('../mis_tools/category_selector.jsp?Back_id=pd_category&Back_category=pd_upcategory&max_layer=<%=category_num %>&dm_code=<%=code+"_category" %>','_blank','height=260,width=400,top=50,left=300,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');" />
                  					<% } %>
                  					<%--
									//兩層下拉
									<select name="pd_category" id="pd_category">
										...
										<optgroup label="<%=dm.getString("dm_title")%>">
											...
											%>
											<option value="<%=sub_dm.getString("dm_id")%>" <%=pd.getString("pd_category").equals(sub_dm.getString("dm_id")) %>><%=sub_dm.getString("dm_title")%></option>
											<%} %>
										</optgroup>
										<%} %>
									</select>
		                    ...
		                    <tr class="web_table-2-1">
								<td colspan="3" align="left" class="tablebg">
									...
									<%-- 原始圖檔資訊 --%>
								    <input name="pd_oldImage<%=i %>" id="pd_oldImage<%=i %>" type="hidden" value="<%=pi.getString("pi_image")%>">
						...
						<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
							...
							<tr class="web_table-2-1">
								...
								<td align="left" class="tablebg">
									<input name="pd_emitdate" id="_qemitdate" type="text" value="<%=pd.getString("pd_emitdate") %>" size="15" readonly>
								</td>
								<td align="right" class="tablebg">下架日期</td>
								<td align="left" class="tablebg">
									<input name="pd_restdate" id="_qrestdate" type="text" value="<%=pd.getString("pd_restdate") %>" size="15" readonly>
								</td>
							</tr>
							<%} %>
							<tr class="web_table-2-1">
