...
/*----------------------------------------------------------------------------------*/
	Vector nps = app_sm.selectAll(tblnp, "np_code=? and np_lang=?", new Object[] { code,lang }, "np_emitdate DESC  , np_createdate DESC");
	...
	if (search_switch) {
		...
		sb.append("np_lang=? and np_code=? and np_category like ?  and np_title like ?");
		keys.add(lang);
		keys.add(code);
		keys.add("%" + qcategory + "%");
		keys.add("%" + qtitle + "%");
		...
		keys.add(qemitdate);
		keys.add(qrestdate);		
		nps = app_sm.selectAll(tblnp, sb.toString(), keys.toArray(), "np_emitdate DESC  , np_createdate DESC");
	}
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0">
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<tr class="web_table-2-1">
						 	<td width="25%" align="center" class="tablebg">所屬類別</td>
                 			<td width="35%" align="center" class="tablebg">標題名稱</td>
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0">
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<tr class="web_table-2-1">
							...
							<td align="center"><%=app_sm.select(tbldm, np.getString("np_upcategory")).getString("dm_title")+"--"+app_sm.select(tbldm,np.getString("np_category")).getString("dm_title") %></td>
							...
							<td align="center">
								...
								<%} %>
								<input type="button" value="文件下載" onclick="goaction(this.form, '<%=code%>_download.jsp?_qcategory=<%=np.getString("np_id") %>');" />&nbsp;
							</td>
						</tr>
						</form>
						<%} %>
