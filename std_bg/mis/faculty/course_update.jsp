...
try {
	...
	if("D".equals(action)) {
		...
		if (app_sm.success()) {
			...
			// 檔案下載
	        Vector<TableRecord> fd_files=app_sm.selectAll(tblfd,"fk_id=?",new Object[]{cp.getString("cp_id")},"fd_showseq ASC , fd_createdate DESC");		   	
	...
	if("A".equals(action)) {		
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				}else if(fi.getFieldName().startsWith("cp_newFileTitle")){ // 相關檔案下載標題
	...
	// 修改
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				}else if(fi.getFieldName().startsWith("cp_FileTitle")){ // 相關檔案下載標題
					fd_filetitle = fieldvalue.trim();
					// 新開檔案 非同一類檔案
					same_file = false;
					file_showseq=0;
