		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("np_title")) {
					Vector checks = app_sm.selectAll(tblnp, "np_category=? and np_title=? and np_code=? and np_lang=?",
							new Object[]{ np.getString("np_category"), fieldvalue, code, lang });
					if(checks.size() > 0) {
	...
	// 修改
		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("np_title")) {
					Vector checks = app_sm.selectAll(tblnp, "np_category=? and np_title=? and np_id<>? and np_code=? and np_lang=?",
							new Object[]{ np.getString("np_category"), fieldvalue, np_id, code, lang });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
