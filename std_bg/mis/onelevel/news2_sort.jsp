...
/*-------------------------------------------------------------------------*/
	...
	nps = app_sm.selectAll(tblnp, "np_code=? and np_lang=? and np_category=?", new Object[] { code,lang,qcategory }, "np_showseq ASC , np_createdate DESC");
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0">
					<tr>
						...
						<table width="100%" border="0" align="center" cellpadding="3"
							...
							<form name="frm" id="frm" method="post" enctype="multipart/form-data" action="<%=code%>_update.jsp?action=S&_qcategory=<%=qcategory %>" onsubmit="javascript:return checkformsequ(this);">
							<tr class="web_table-2-1">
								<td class="web_table-2-1" align="center"><select
									name="left_select" id="left_select" multiple="multiple"
									style="width: 760px;" size="15">
									<%
										for (int i = 0; i < nps.size(); i++) {
											TableRecord np = (TableRecord) nps.get(i);
									%>
									<option value="<%=np.getString("np_id")%>"><%=np.getString("np_title")%></option>
									<%
										}
									%>
								</select></td>
								<td class="web_table-2-1" align="center" colspan="3"><input
									type="hidden" name="selData" id="selData" value=""> <input
									type="button" id="select_up" value="↑↑上移↑↑" /><br />
								<br />
								<input type="button" id="select_down" value="↓↓下移↓↓" /><br />
								<br />
