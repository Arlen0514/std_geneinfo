...
try {
	...
	if("D".equals(action)) {
		...
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+sp.getString("sp_image"));
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+sp.getString("sp_mobile"));
		app_sm.delete(sp);

		//確認是否有其它資料顯示相同圖片(有其它檔案欄位再新增code)
		if (app_sm.success()) {
			if(app_sm.selectAll(tblsp, "sp_image=?", new Object[] { sp.getString("sp_image") }).size() == 0) {
				FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+sp.getString("sp_image"));
			}
			// 回列表頁
	...
	if("S".equals(action)) {
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){											// 這是用來確定是否為文件屬性 
				...
				if("selData".equals(fieldName)){chk_sp = fieldvalue.trim();}
			}
		}
		String list[] = chk_sp.split(",");
		for(int j=0;j<list.length;j++){
			if(!"".equals(list[j].trim())){
				TableRecord sp = app_sm.select(tblsp, "sp_id=?",new Object[] { list[j] });
				sp.setValue("sp_showseq", j);
				sp.setUpdate(app_account);
				app_sm.update(sp);
			}
		}
	...
	// 新增
	if("A".equals(action)) {
		TableRecord sp = new TableRecord(tblsp);
		sp.setInsert(app_account);
		//---------------------檔案上傳-----------------------------------------------
		while (i.hasNext()) {
			FileItem fi = (FileItem) i.next();
			...
			if(fi.isFormField()) {  
				...
				if(fieldName.equals("sp_title")) {
					/*
					Vector checks = app_sm.selectAll(tblsp, "sp_category=? and sp_title=? and sp_code=? and sp_lang=?",
							new Object[]{ sp.getString("sp_category"), fieldvalue, code, lang });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
					*/
					sp.setValue(fieldName, fieldvalue.trim());			// 設定欄位值
				} else if(fieldName.contains("sp_")) {
					sp.setValue(fieldName, fieldvalue.trim());			// 設定欄位值
				}
			...
			// 處理文件
				...
				//fileName_1 = sp.getString("sp_id")+"_"+fileName_1;
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					sp.setValue(fi.getFieldName(), fileName_1);//設定圖
				}
			}
		}
		sp.setValue("sp_code", code);	// 識別碼
		sp.setValue("sp_lang", lang);	// 語系		
		app_sm.insert(sp);
	...
	// 修改
		...
		TableRecord sp = app_sm.select(tblsp, "sp_id=?",new Object[] { sp_id });
		sp.setUpdate(app_account);
		while (i.hasNext()) {
			FileItem fi = (FileItem) i.next();
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("sp_title")) {
					/*
					Vector checks = app_sm.selectAll(tblsp, "sp_category=? and sp_title=? and sp_id<>? and sp_code=? and sp_lang=?",
							new Object[]{ sp.getString("sp_category"), fieldvalue, sp_id, code, lang});
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
			...
			// 處理文件(file)
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					if(!"".equals(sp.getString(fi.getFieldName()))){
						//確認其它資料沒有使用相同檔案
						if(app_sm.selectAll(tblsp, fi.getFieldName()+"=?", new Object[] {sp.getString(fi.getFieldName()) }).size() == 1) {
							FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+sp.getString(fi.getFieldName()));
						}
					}
