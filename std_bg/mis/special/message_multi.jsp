...
<%
	// 基本參數
	String code = "message_multi"; 			// 模組識別碼
	String upload_code = "about"; 			// 上傳資料夾識別碼
	String show_title = "多封簡訊寄送設定"; 		// 模組標題
	...
	//所有會員
	Vector<TableRecord> mps = app_sm.selectAll(tblmp, "mp_status=? and mp_cellphone<>?"
			, new Object[]{ "Y", ""});
%>
...
<script>
	function checkMessage(F) {
		...
		var memberChoice = new Array();
		$('input:checkbox:checked[name="_memberChoice"]').each(function(i) { memberChoice[i] = this.value; });
		F._cellphone.value = memberChoice;
...
<%-- 20250331 modify Miles top menu.jsp  改成 RWD --%>
					<form name="frm" id="frm" method="post" <%--enctype="multipart/form-data"--%> action="../../web/message/every8d_message_post.jsp?action=multi" onsubmit="javascript:return checkMessage(this);">
												...
												<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
													...
													<tr class="web_table-2-1">
														...
														<td colspan="3" width="85%" align="left">
															...
															<input name="_cellphone" id="_cellphone" type="hidden" value="" size="25"/> <%-- 手機號碼整合用 --%>
															<%
															int i = 0;
															for(TableRecord mp:mps) { 
															%>
																<input name="_memberChoice" type="checkbox" value="<%=mp.getString("mp_cellphone") %>" checked/>
																<%="會員姓名: " + mp.getString("mp_name") + ", 手機號碼:" + mp.getString("mp_cellphone") %>
																<br />
															<%} %>
														</td>
													</tr>
