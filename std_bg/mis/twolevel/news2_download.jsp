...
/*--------------------------------------------------------------------------------------------*/
	...
	Vector cps = app_sm.selectAll(tblcp, "cp_code=? and cp_lang=? and cp_category like ?"
			, new Object[] { code, lang ,qcategory}, "cp_showseq ASC , " + "cp_createdate DESC");
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<tr>
							<td align="center" colspan="3" class="web_title-1"><%=show_title%>&nbsp;&nbsp;
								...
								<%} %>
								<%if (list_switch) { %>
								<input type="button" value="回最新消息" onclick="javascript:location.href='<%=back_code%>.jsp?'" />&nbsp;
								<%} %>
								<%if (sort_switch) { %>
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<tr class="web_table-2-1">
							...
							<td align="center">
								...
								<%if (modify_switch) { %>
								<input type="button" value="修改" onclick="goaction(this.form, '<%=code%>_c.jsp?_qcategory=<%=qcategory %>');" />&nbsp;
								<%} %>
								<%if (delete_switch) { %>
								<input type="button" value="刪除" onclick="godelete(this.form, '<%=code%>_update.jsp?action=D&_qcategory=<%=qcategory %>');" />
								<%} %>
							</td>
						</tr>
						</form>
