...
/*--------------------------------------------------------------------------------------------*/
	Vector sps = app_sm.selectAll(tblsp, "sp_code=? and sp_lang=?", new Object[] { code, lang }, "sp_showseq ASC , " + "sp_createdate DESC");
	// 當資料筆數小於設定可新增的筆數時 , 顯示新增按鍵
	boolean add_switch = num_check(add_num,sps);
	// 當資料筆數小於設定可刪除的筆數時 , 隱藏刪除按鍵
	boolean delete_switch = num_check(del_num,sps);	
	...
	if(search_switch) {
		...
		sb.append("sp_lang=? and sp_code=? and sp_category like ? and sp_title like ?");
		keys.add(lang);
		keys.add(code);
		keys.add("%" + qcategory + "%");
		keys.add("%" + qtitle + "%");			
		sps = app_sm.selectAll(tblsp, sb.toString(), keys.toArray(), "sp_showseq ASC , sp_createdate DESC");
	}
	
	// 分頁
	out.write(HtmlCoder.getForm("pageform", request.getRequestURI(), names, values));
	// 分頁設定
	app_dp = new DataPager(sps, page_items);
	sps = app_dp.getPageContent(pageno);
%>
<!DOCTYPE html>
<html>
<head>
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<%
							for (int i = 0; i < sps.size(); i++) {
								TableRecord sp = (TableRecord) sps.get(i);
						%>
						<form name="list<%=i + 1%>" id="list<%=i + 1%>" method="post">
						<tr class="web_table-2-1">
							<td align="center"><%=((pageno - 1) * page_items) + i + 1%></td>
							<td align="center"><%=app_sm.select(tbldm, sp.getString("sp_upcategory")).getString("dm_title")+"--"+app_sm.select(tbldm,sp.getString("sp_category")).getString("dm_title") %></td>
							<td align="center"><%=sp.getString("sp_title") %></td>							
							<td align="center">
								<%=HtmlCoder.hiddenInputs(names, values)%>
								<input type="hidden" name="sp_id" id="sp_id" value="<%=sp.getString("sp_id")%>" />
								<%if (modify_switch) { %> 
