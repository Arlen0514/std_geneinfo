	...
	// 修改
		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("ap_title")) {
					Vector checks = app_sm.selectAll(tblap, "ap_category=? and ap_title=? and ap_id<>? and ap_code=? and ap_lang=?",
							new Object[]{ ap.getString("ap_category"), fieldvalue, ap_id, code, lang });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
