		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("pd_title")) {
					Vector checks = app_sm.selectAll(tblpd, "pd_category=? and pd_title=? and pd_code=? and pd_lang=?",
							new Object[]{ pd.getString("pd_category"), fieldvalue, code, lang });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
					pd.setValue(fieldName, fieldvalue.trim());			// 設定欄位值
				} else if(fieldName.contains("pd_")) {
					pd.setValue(fieldName, fieldvalue.trim());			// 設定欄位值	
				}
	...
	// 修改
		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("pd_title")) {
					Vector checks = app_sm.selectAll(tblpd, "pd_category=? and pd_title=? and pd_id<>? and pd_code=? and pd_lang=?",
							new Object[]{ pd.getString("pd_category"), fieldvalue, pd_id, code, lang });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
