...
<%
	...
	// 圖片建議尺寸
	String image_info = "(建議尺寸366px * 240px)";
	// 圖片建議尺寸
...
/*-------------------------------------------------------------------------------------*/
	...
	// 所屬類別標題
	String title1=app_sm.select(tbldm,ap.getString("ap_upcategory")).getString("dm_title");
	String title2=app_sm.select(tbldm,ap.getString("ap_category")).getString("dm_title");
%>
<html lang="<%=encoded %>">
<head>
...
function checkform(F) {
	...
	if(F.ap_category.value == "") {
        alert("請選擇類別!!");
        F.ap_category.focus();
	} else if(F.ap_title.value == "") {
        alert("請輸入標題名稱!!");
        F.ap_title.focus();	
	} else if(tinyMCE.get("ap_desc").getContent() == "") {
		alert("請輸入簡述!!");
		$("#ap_desc").focus();
...
<%-- 20250331 modify Miles top menu.jsp  改成 RWD --%>
			...
			<tr>
				<form name="frm" id="frm" method="post" enctype="multipart/form-data" action="<%=code%>_update.jsp?action=M&ap_id=<%=ap_id %>&_qtitle=<%=qtitle %>&_qcategory=<%=qcategory %>&npage=<%=pageno %>&_qupcategory=<%=qupcategory %>" onsubmit="javascript:return checkform(this);">
						...
						<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
							...
							<tr class="web_table-2-1">
								...
								<td colspan="3" width="85%" align="left">
							  		<% if(category_num<=1){ %>
								    <select name="ap_category" id="ap_category">
										<option value="">請選擇類別</option>
										<% for(TableRecord dm : dms){ %>
											<option value="<%=dm.getString("dm_id")%>" <%=dm.getString("dm_id").equals(ap.getString("ap_category"))?"selected":""%>><%=dm.getString("dm_title")%></option>
										<% } %>
									</select>
									<% }else{  //2層以上 類別選擇器 %>
									<span id="dm_title" style="color:#FA0300;"><%=title1+"--"+title2%></span>&nbsp;&nbsp;
                   					<input type="hidden" name="ap_upcategory" id="ap_upcategory"  value="<%=ap.getString("ap_upcategory") %>"/>
									<input type="hidden" name="ap_category" id="ap_category"  value="<%=ap.getString("ap_category") %>"/>
                  					<input type="button" value="選擇類別" onclick="window.open('../mis_tools/category_selector.jsp?Back_id=ap_category&Back_category=ap_upcategory&max_layer=<%=category_num %>&dm_code=<%=code%>_category','_blank','height=260,width=400,top=50,left=300,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');" />
                  					<% } %>
                  					<%--
									//兩層下拉
									<select name="ap_category" id="ap_category">
										...
										<optgroup label="<%=dm.getString("dm_title")%>">
											...
											%>
											<option value="<%=sub_dm.getString("dm_id")%>" <%=ap.getString("ap_category").equals(sub_dm.getString("dm_id")) %>><%=sub_dm.getString("dm_title")%></option>
											<%} %>
										</optgroup>
										<%} %>
									</select>
							...
							<tr class="web_table-2-1">
								...
								<td colspan="3" align="left">
									<textarea name="ap_desc" id="ap_desc" cols="100" rows="8" class="ezEditor"><%=ap.getString("ap_desc")%></textarea>
								</td>
							</tr>
							...
							<tr class="web_table-2-1">
						        ...
						        <% if(!ap.getString("ap_image").isEmpty()){ %>
									<img src="<%=app_fetchpath+"/"+code+"/"+lang+"/"+ap.getString("ap_image")%>" width="366">		                      		
							    <% } %>
								</td>
		                    </tr>
		                    <tr class="web_table-2-1">
