...
try{
	...
	if("D".equals(action)) {
		...
		Vector<TableRecord> cps =  app_sm.selectAll(tblcp, "cp_code=? and cp_lang=? and cp_category=?", new Object[]{ code+"_download", lang, np_id });
		for(TableRecord cp:cps) {
			app_sm.delete(tblfd, "fk_id=?", new Object[]{ cp.getString("cp_id") });
		}
		app_sm.delete(tblcp, "cp_code=? and cp_lang=? and cp_category=?", new Object[]{ code+"_download", lang, np_id });
		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("np_title")) {
					Vector checks = app_sm.selectAll(tblnp, "np_category=? and np_title=? and np_code=?",
							new Object[]{ np.getString("np_category"), fieldvalue, code });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
					np.setValue(fieldName, fieldvalue.trim());			// 設定欄位值
				} else if(fieldName.contains("np_")) {
					np.setValue(fieldName, fieldvalue.trim());			// 設定欄位值
			    }
	...
	// 修改
		...
		while (i.hasNext()) {
			...
			if(fi.isFormField()) {
				...
				if(fieldName.equals("np_title")) {
					Vector checks = app_sm.selectAll(tblnp, "np_category=? and np_title=? and np_id<>? and np_code=?",
							new Object[]{ np.getString("np_category"), fieldvalue, np_id, code });
					if(checks.size() > 0) {
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
