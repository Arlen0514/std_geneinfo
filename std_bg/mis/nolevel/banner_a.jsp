...
<script>
	function checkform(F) {
		...
		if(F.ap_title.value == "") {
	        ...
	        F.ap_image.focus();
	        <%--
	    } else if(chnese_chk.test(F.ap_image.value.toLowerCase())) {
	        alert("檔案名稱不可有包含中文!!");
	        F.ap_image.focus();
	        ...
	        --%>
	    <% if(mobile_image){ %>
	    } else if(F.ap_mobile.value == "") {
	        alert("請上傳手機輪播圖檔!!");
	        F.ap_mobile.focus();
	    } else if(!file_chk.test(F.ap_mobile.value.toLowerCase())) {
	    	alert("附檔名限為jpg|jpeg|gif|png|webp!!");
	        F.ap_mobile.focus();
	    <% } %>
	        <%--
	    } else if(chnese_chk.test(F.ap_mobile.value.toLowerCase())) {
	        alert("檔案名稱不可有包含中文!!");
	        F.ap_mobile.focus();
	    } else if(mobile.size/1024><%=fSize%>) {
	        alert("<%=file_info%>!!");
	        F.ap_mobile.focus();
	         --%>
							...
							<td class="system_bk-2bk"><table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
								...
								<tr class="web_table-2-1">
									...
									<td colspan="3" align="left">
										<textarea name="ap_alt" id="ap_alt" cols="100" rows="8"></textarea>
									</td>
								</tr>
								...
								<tr class="web_table-2-1">
									...
									<td colspan="3" align="left">
										<input type="text" name="ap_url" id="ap_url" size="100"/>
									</td>
								</tr>
