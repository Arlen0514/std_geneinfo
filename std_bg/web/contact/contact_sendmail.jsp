...
<%
	...
	if("".equals(cu_id)) {
		...
		String servmailto = cu.getString("cu_email"); 										// 設定正本收件人 e-Mail
		...
		subject = SiteSetup.getText("cp.company." + lang) + " 聯絡我們通知信";
		...
		if(url.indexOf("https://")>-1) {
			content = SSLreturnContent(url + "/web/mail/contact_mail.jsp?cu_id=" + cu_id + "&lang=" + lang);
		} else {
		...
		try {
			...
			System.out.println("Project:" + projectName + ", Error info:["+ex+"], File name:web/contact/contact_sendmail.jsp, Time:["+DateTimeTool.dateTimeString()+"]");
